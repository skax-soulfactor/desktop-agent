      ...
      parts.push({
        ...
        text: path
          ? `[첨부 "${att.name}" (${att.mimeType || '알 수 없는 형식'})은 네가 직접 읽을 수 있는 형식이 아니다. ` +
            // 어떤 명령으로 볼지는 적지 않는다. 셸이 무엇인지는 shell_exec 설명에 있고,
            // 여기서 유닉스 명령을 예로 들면 cmd.exe에서 없는 명령을 권하는 꼴이 된다.
            `아래 경로에 실제 파일이 있으니 그 형식에 맞는 도구·명령으로 열어 처리하라. ` +
            `못 한다고 답하기 전에 파일을 먼저 봐라.]`
